...
//!
//! - An `allow.read` entry that does not exist is **dropped**, with a warning.
//!   A grant for a path that is not there never reaches the sandbox.
//! - A `deny.paths` entry that cannot be resolved is a **hard error** that
//!   stops the launch: "Silently dropping deny rules is a security risk." That
//!   asymmetry with `allow.read` is the tighten-only rule doing its job, and it
//!   is the single most important thing in this file to keep.
//! - Paths are **canonicalized**, so `/etc` resolves to `/private/etc` on
//!   macOS. What reaches the sandbox is the resolved path, not the text.
//!
//! So the cases below use paths that exist on any machine (`/usr`, `/etc`)
//! rather than invented ones. A corpus that quietly asserted on dropped grants
...
fn resolve(case: &Case) -> Resolved {
    ...
    global
        .merge_local_with_no_proxy_env(
            local.as_ref(),
            (case.cli)(),
            case.no_proxy_env.map(str::to_string),
        )
        .unwrap_or_else(|e| panic!("{}: resolution should succeed: {e}", case.name))
}
...
const CORPUS: &[Case] = &[
    ...
    Case {
        ...
        expect: |r| {
            ...
            // canonicalizes — which is itself a recorded behaviour: the grant
            // that reaches the sandbox is the resolved path, not the text.
            assert!(
                read.iter().any(|p| p.ends_with("etc")),
                "the local entry must be present: {read:?}"
            );
        },
    },
    Case {
...
fn corpus_resolves_as_recorded() {
    ...
    for case in CORPUS {
        ...
        if let Err(panic) =
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| (case.expect)(&resolved)))
        {
            let message = panic
                .downcast_ref::<String>()
                .cloned()
...
fn resolution_is_deterministic() {
    for case in CORPUS {
        let first = resolve(case);
        let second = resolve(case);
        assert_eq!(
            format!("{:?}", first.deny_paths),
            format!("{:?}", second.deny_paths),
            "{}: resolving twice gave different deny paths",
            case.name
        );
        assert_eq!(
            first.allow_docker, second.allow_docker,
            "{}: resolving twice gave different allow_docker",
            case.name
        );
        assert_eq!(
            first.git_guard.mode, second.git_guard.mode,
            "{}: resolving twice gave different git guard mode",
            case.name
        );
    }
}
