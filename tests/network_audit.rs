...
mod tests {
    ...
    fn audit_switch_preserves_blocked_connect_outcomes() {
        ...
        for audit in [true, false] {
            let project = project();
            // Not inside the project, and not `tempfile::tempdir()` either: a
            // list file in any tree the session can write is refused at launch,
            // because the proxy re-reads it every few seconds and the agent
