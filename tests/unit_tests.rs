    ...
    temp_env::with_var_unset("CLAUDE_CONFIG_DIR", || {
        ...
        for agent in [
            ...
            let p = generate_profile(&ProfileOptions {
                ...
                allow_cache_exec_any: false,
                allow_browser: false,
                // Preserves main's behaviour: keychain followed needs_keychain()
                // before this branch made it explicit. Unrelated to what this test asserts.
                allow_keychain: agent.needs_keychain(),
            });
