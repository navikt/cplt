...
- **The Gemini CLI agent was removed.** Google deprecated it in favour of Antigravity, so `--agent gemini` now errors and points at `--agent agy`. Antigravity is unaffected: its grants are `~/.gemini/config` and `~/.gemini/antigravity-cli`, not `~/.gemini` itself.
