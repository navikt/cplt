...
| `--allow-port <PORT>` | Allow outbound TCP on an extra port. Only 443 by default. Repeatable. Under `proxy.forced` the port is reachable through the proxy only, not by a direct socket |
