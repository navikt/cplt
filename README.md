...
| Auth tokens       | `GH_TOKEN`, `GITHUB_TOKEN`, `COPILOT_GITHUB_TOKEN`                         | Passed through if set by user. On macOS + Copilot, `copilot_auth` (default `auto`) may pre-extract a token via `gh` and inject it as `GH_TOKEN` even when none is set. With `gh_guard.block_auth_token`, the token is normally served once from a 0600 file for `gh auth token` (and may be withheld from env unless `inject_token=true` or the token must be injected because Keychain access is blocked). Repo `deny.env` on `GH_TOKEN` overrides all of this. |
