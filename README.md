...
`goose` and `pi` share their names with unrelated tools (pressly/goose is a Go migration tool), so `--shell-install --shims` and `cplt doctor` check their `--version` output before shimming them, and remember a binary that failed. If it still picks up the wrong binary, leave the agent out: `cplt config set shell.skip goose`.
