...
- **Host-persistence guard on the agent's own config dir.** Every agent gets its global config dir mounted read/write, and a few files in there auto-execute the next time the agent runs *outside* the sandbox. Those specific paths are write-denied: `statusline.sh`, `plugins/` and `settings.json` for Claude Code, `settings.json` and `extensions/` for Gemini and Pi (`Agent::host_persistence_denies`). On **macOS** this is a Seatbelt `(deny file-write*)` emitted after the dir-wide allow, so SBPL last-match-wins enforces it. On **Linux** Landlock cannot deny a subpath inside an allowed directory, so the deny is carried by the same bubblewrap read-only overlay used for `.git/hooks`: it holds only when `bwrap` is present, and only for paths that **already exist on disk** — bubblewrap cannot bind a missing source, so a file the agent creates for the first time inside the sandbox is not covered. Without bubblewrap, this guard is unenforced on Linux.
- **Cost: log in once outside cplt.** Denying `settings.json` breaks first-run setup that writes to it. For **Gemini** that is sign-in itself: it records the auth method you pick as `selectedAuthType` in that file. For **Pi** it is package management only — Pi authenticates from provider API keys and has no interactive login — and for **Claude Code** nothing, since its OAuth token lives in the Keychain or `~/.claude/.credentials.json`. Do the affected step once in a normal, unsandboxed run.
- **cplt refuses to launch rather than let you meet this as a denied write.** Before starting Gemini, cplt checks whether sign-in has already happened (`~/.gemini/oauth_creds.json` exists, or `settings.json` already records a `selectedAuthType`) and, if not, exits with instructions instead of launching into an agent that will fail with an opaque permission error. The check is skipped when a provider API key is passed with `--pass-env` (that authenticates without touching the file), under `--inherit-env`, and in `cplt check`. It is two filesystem probes and never fails a launch on an I/O error: anything it cannot read is treated as authenticated.
- **Escape hatch, and where it works.** A user `allow.write` grant beats the persistence deny on **macOS**: `emit_user_allows` is emitted after the agent config-dir denies, so SBPL last-match-wins reopens the file. `cplt --agent gemini --allow-browser --allow-write ~/.gemini/settings.json` therefore completes a login under cplt. On **Linux** it does not: the deny is a bubblewrap read-only bind emitted after every writable bind, and no allow can override a mount. Linux users log in outside cplt — which is usually moot, since a `settings.json` that does not exist yet is not protected there at all.
...
**Gemini.** First-time OAuth login needs `--allow-browser`, and has to be done **outside cplt** (cplt refuses to launch an unauthenticated Gemini rather than let the login fail as a denied write — see above): `~/.gemini/settings.json` is write-denied by the host-persistence guard and Gemini writes `selectedAuthType` there on first login. `~/.gemini/` otherwise holds auth, settings, sessions, and agents read/write. `settings.json` and `extensions/` are the denied paths. `settings.json`'s `hooks.SessionStart[].hooks[].command` entries run with the user's privileges the next time `gemini` starts *outside* the sandbox — `SessionStart` fires on application startup, on resume, and after `/clear` — and user-level hooks are not gated by folder trust, which covers workspace (`.gemini/`) settings, custom commands, MCP servers and extensions, and does not disable `~/.gemini/settings.json`. **Deliberate residual: `trustedFolders.json` stays writable.** This is a decision, not an oversight — a sandboxed agent has to be able to trust the folder it is working in, and denying the file would break folder trust from inside cplt. It is the unsigned central record of per-folder trust decisions, so the trade is real: an agent can pre-trust a directory and suppress the dialog that would otherwise disclose *that project's* hooks. What that buys an attacker is now narrower than it was. It no longer gets them a hook of their own, because `~/.gemini/settings.json` is denied and workspace `.gemini/` settings live in the project tree, which the audit surfaces as a diff. What it still buys is silence: a `.gemini/settings.json` that arrived some other way — a dependency, a branch, a repo you cloned — runs without the trust prompt that would have shown it to you. Review project-level `.gemini/` config yourself; do not rely on the dialog. The `settings.json` deny is also subject to the Linux limits described above.
...
**Pi.** Not auto-detected: the `pi` binary name is generic and may collide with other tools, so select it explicitly with `--agent pi` or `sandbox.agent = "pi"`. `~/.pi/agent/` holds settings, auth, sessions, and themes. The managed binary dir `~/.pi/agent/bin/` (bundled `fd`, `rg`) has process-exec, and on **macOS only** an explicit `(deny file-write*)` follows the parent allow, so SBPL last-match-wins blocks write+exec persistence even though the parent is writable. Linux has no equivalent: Landlock rules are additive, so the parent's read+write unions with the child's read+exec and `~/.pi/agent/bin/` ends up **writable and executable**. `~/.pi/agent/extensions/` and `~/.pi/agent/settings.json` are write-denied by the host-persistence guard (macOS Seatbelt; on Linux only via the bubblewrap overlay, and only for paths that already exist). Pi auto-discovers `~/.pi/agent/extensions/*.ts` and `*/index.ts` and loads them at startup without confirmation, since the project-trust gate covers only project-local `.pi/extensions`, and Pi's own documentation states that extensions run with the user's full system permissions and can execute arbitrary code. `settings.json` in the same directory is denied for the same reason: its `extensions` key loads code from arbitrary file paths and its `packages` key from npm or git, so denying the `extensions/` directory alone would not close the vector. A file written to any of these would execute unsandboxed the next time `pi` runs on the host. The cost is that in-sandbox package management no longer works — install packages outside cplt.
...
| **4a. HTTP exfil** | POST to discord/webhook/C2 | Only port 443 allowed. On macOS localhost and the SSH agent socket are blocked too; on Linux neither is, and UDP is unrestricted (see [Linux-specific limitations](#linux-specific-limitations)). Credentials are unreadable, so the blast radius is small. The proxy blocklist helps if enabled. | ⚠️ **Partially mitigated** |
...
- A compromised agent CANNOT use loaded SSH keys **on macOS**, where `(deny default)` plus the per-path `network-outbound … unix-socket` allows leave the agent socket unreachable at the kernel level. **On Linux it CAN**: unix socket `connect()` is not gated at the Landlock ABI cplt targets, so the withheld `SSH_AUTH_SOCK` is the only barrier and an agent that sets it itself can use the loaded keys, see [Linux-specific limitations](#linux-specific-limitations)
...
- **Linux (Landlock):** Landlock drops the `:443` rule and allows only the proxy port. This blocks direct **TCP** `:443` to any host and forces HTTPS through the proxy, but two things stay open. Landlock is **port-based and cannot pin to localhost**, so a narrow `evil.com:<proxy_port>` channel stays reachable if a remote host answers on that exact port. And the restriction is **TCP-only**: Landlock gates UDP only at ABI v10 and cplt handles `AccessNet::ConnectTcp` alone, so outbound UDP to any host and port is unrestricted at the kernel in every Linux mode, proxy-forced included. DNS-tunnelled exfiltration, QUIC/HTTP-3 and plain UDP all leave without touching the proxy. This is "no direct TCP `:443` bypass", **not** "no egress except the proxy". Closing both residuals requires a network namespace, tracked in [#114](https://github.com/navikt/cplt/issues/114).
...
cplt requires ABI v1 minimum and clamps the requested ABI at v6. On ABI < v4, network security relies on the CONNECT proxy alone, because Landlock cannot filter TCP ports. On ABI v4+, Landlock denies all TCP connections except to explicitly allowed ports.

Two consequences of the clamp, both unmitigated: **UDP is never restricted**, since Landlock's UDP support lands at ABI v10 and cplt handles `AccessNet::ConnectTcp` only, and **`connect()` to a pathname unix socket is never restricted**, since that lands at ABI v9 (kernel 7.1). The seccomp filter blocks no socket syscall either, so neither gap has a second layer behind it on Linux.
...
5. **Unix sockets are not gated at all.** Landlock cannot restrict `connect()` to a pathname unix socket before ABI v9 (kernel 7.1) and cplt clamps at v6, the seccomp filter blocks no socket syscall, and a read-only bubblewrap bind does not stop a `connect()` either. Any unix socket whose path the agent can name is reachable: the SSH agent socket, `/var/run/docker.sock`, the gpg-agent socket, `$XDG_RUNTIME_DIR/bus`.
   ...
   So `--allow-socket`'s Landlock rules and the `--allow-gpg-signing` socket rules are **no-ops on Linux**, granting filesystem access that `connect()` never consults, and the D-Bus path below is reachable by default.
6. **D-Bus reaches the session manager.** `XDG_RUNTIME_DIR` is on the env allowlist and `$XDG_RUNTIME_DIR/bus` is connectable (see above), so `systemd-run --user <cmd>` hands the command to the user's systemd instance, which starts it as a fresh unit **outside** Landlock, seccomp and the bubblewrap namespaces. The path is open with and without bwrap. This is established by reading the code (the env allowlist, the absence of a network namespace, `--ro-bind / /` exposing `/run/user/<uid>/bus`, no seccomp socket filtering, and no Landlock unix-connect right below ABI v9), not by running it end to end on a Linux host, so treat it as reachable rather than demonstrated. cplt has no mitigation today; if it matters for your threat model, run cplt inside a container or a VM.
7. **UDP is unrestricted.** Landlock gates UDP only at ABI v10, and cplt handles `AccessNet::ConnectTcp` only. Outbound UDP to any host and port, and inbound UDP bind, are unrestricted at the kernel in **every** Linux mode including proxy-forced. The CONNECT proxy carries TCP only, so UDP never touches it, and DNS tunnelling, QUIC/HTTP-3 and plain UDP exfiltration are covered by no cplt layer on Linux. macOS restricts UDP, since SBPL's `remote ip` covers UDP as well as TCP and `(deny default)` closes everything the profile does not name, but does not route it through the proxy: in default mode `*:443` admits QUIC/HTTP-3, so the proxy log is a complete record only under `proxy.forced`, where that allow is dropped.
...
**Platform note:** on macOS this grant is what makes the socket reachable — without `--allow-gpg-signing` the profile's `(deny default)` blocks the `connect()`. On Linux the grant changes nothing: Landlock cannot restrict `connect()` to a pathname unix socket at the ABI cplt targets, so the gpg-agent socket is reachable whether or not the flag is set. The read denies on `~/.gnupg/private-keys-v1.d/` still hold on both platforms — but on Linux the "signature impersonation and decryption" risk described below is present by default, not opted into.
