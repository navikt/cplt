...
| **Org/user data enumeration** | `gh api /orgs/.../audit-log` leaks PII | gh guard restricts API to `/repos/{current-repo}/...` endpoints only. GraphQL queries must start at a repository in scope. That is weaker: see [GraphQL reads are scoped at the root](#honest-gaps) |
...
**GraphQL reads are scoped at the root.** `gh api graphql` queries must start at `repository(owner:, name:)` for a repository in scope (#414). REST reads are held to `/repos/{owner}/{repo}/…` one request at a time, but a GraphQL query can follow edges out of the repository. The gate refuses a short list of fields that lead to the owner, organization, members, audit log or other repositories. That list is not exhaustive: comment authors, cross-referenced issues in other repositories and a fork's head repository are still readable wherever the token can read them. The three allowed mutations are checked more tightly, since each target node is looked up and must belong to a repository in scope. The lookup runs inside the sandbox with the agent's own `gh`, so like the rest of the guard it binds a cooperative agent, not a hostile one.
