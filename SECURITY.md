...
\* Exception: `~/Library/Caches/copilot/pkg/` has `file-map-executable` and `process-exec` for Copilot's native modules and helper binaries (`pty.node`, `spawn-helper`, `rg`). A `file-write*` deny prevents write-then-exec attacks. The two exec allows sit after the broader deny rules, and the write deny and the rename pin on `~/Library/Caches/copilot` sit after every allow, so an `allow.write` on `~/Library/Caches` cannot reopen them, since SBPL is last-match-wins. When `copilot` or `pkg` resolves elsewhere through a symlink outside every writable tree, the same rules are also emitted at the target; a symlink inside a writable tree, which the agent could have planted and can re-point, stops the launch on both platforms.
