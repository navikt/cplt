...
- A compromised agent CANNOT use loaded SSH keys on macOS, where `(deny default)` plus the per-path `network-outbound … unix-socket` allows leave the agent socket unreachable at the kernel level. **On Linux this is not kernel-enforced.** Landlock cannot restrict `connect()` to a pathname unix socket before ABI v9 (kernel 7.1) and cplt caps at v6, and the seccomp filter blocks no socket syscall. The only barrier is that `SSH_AUTH_SOCK` is withheld from the environment, so an agent that finds the socket (`ls /tmp/ssh-*/agent.*`, which a readable `/tmp` permits) and sets the variable itself can use the loaded keys. The Bubblewrap layer's private `/tmp` hides the usual location, which makes bwrap sessions materially harder to attack, but that is concealment, not a kernel deny
...
5. **Unix sockets are not gated at all.** Landlock cannot restrict `connect()` to a pathname unix socket before ABI v9 (kernel 7.1) and cplt clamps at v6, the seccomp filter blocks no socket syscall, and a read-only bubblewrap bind does not stop a `connect()` either. Any unix socket whose path the agent can name is reachable: the SSH agent socket, `/var/run/docker.sock`, the gpg-agent socket, `$XDG_RUNTIME_DIR/bus`. So `--allow-socket`'s Landlock rules and the `--allow-gpg-signing` socket rules are **no-ops on Linux**, granting filesystem access that `connect()` never consults, and the D-Bus escape below is open by default.
6. **D-Bus reaches the session manager, so the sandbox is escapable by default.** `XDG_RUNTIME_DIR` is on the env allowlist and `$XDG_RUNTIME_DIR/bus` is connectable (see above), so `systemd-run --user <cmd>` hands the command to the user's systemd instance, which starts it as a fresh unit **outside** Landlock, seccomp and the bubblewrap namespaces. This holds with and without bwrap. cplt has no mitigation today; if it matters for your threat model, run cplt inside a container or a VM.
7. **UDP is unrestricted.** Landlock gates UDP only at ABI v10, and cplt handles `AccessNet::ConnectTcp` only. Outbound UDP to any host and port, and inbound UDP bind, are unrestricted at the kernel in **every** Linux mode including proxy-forced. The CONNECT proxy carries TCP only, so UDP never touches it, and DNS tunnelling, QUIC/HTTP-3 and plain UDP exfiltration are covered by no cplt layer on Linux. macOS is unaffected: SBPL's `remote ip` covers UDP as well as TCP, and `(deny default)` closes everything the profile does not name.
