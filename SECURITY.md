...
- **macOS:** writes are denied at the `$HOME` path and at the resolved target, after every write grant, so the target stays read-only inside the project. The directories between the outermost writable tree holding the target and the target cannot be renamed or removed, so `mv dotfiles d2 && mkdir dotfiles` cannot swap in a fresh file. The writable root itself (the project, the `--repo-dir` root, the `allow.write` grant) is not pinned: moving it away is harmless, because recreating it needs write on its parent, which no grant gives. If `~/.config/git` is itself a link, `config`, `ignore` and `attributes` at its target are write-denied even when they do not exist yet; a new `config` could set `core.fsmonitor` or `core.hooksPath` and run code in the next git command outside the sandbox. If `~/.config` is the link and has no `git` directory yet, creating `git` there is denied, so it cannot be made a symlink to somewhere writable. A target whose path cannot be written into the profile (it contains `"`, `(`, `;` and the like) and sits in a writable tree stops cplt at launch rather than going unprotected.
- **Linux with Bubblewrap:** the target is bound read-only over the writable tree, and the same directories are pinned as mountpoints, so they cannot be renamed. This is checked by unit tests of the overlay and pin sets, not by an end-to-end kernel test; it is the mechanism the `.git/hooks` overlay already relies on.
- **Linux without Bubblewrap:** not enforced. Landlock can only add grants, so it cannot take the write back from a file inside a writable tree. cplt warns at launch with the file's path.
...
On both platforms the link is resolved once, at launch, and only the file it ends at is protected. Three things fall outside that. A link whose target does not exist at launch is not covered, so the agent can create the file. An intermediate link in a chain (`~/.gitconfig -> link -> file`) that lives in a writable tree can be re-pointed. And on Linux, a missing file cannot be bound read-only, so when `~/.config/git` itself is a link into a writable tree and has no `config`, `ignore` or `attributes` yet, the agent can create them there (macOS denies them in advance). A new `config` can set `core.fsmonitor` or `core.hooksPath`, which run code in the next git command outside the sandbox; `ignore` is git's default excludes file, so an agent can hide files from `git status` through it.
