...
**Concealment, not just execution.** `.git/info/exclude` is a local, uncommitted gitignore. Nothing in it executes, but every path it names disappears from `git status` — for the user and for any tooling that shells out to git — and `git status` is the review step people actually perform. An agent that plants a file and then hides it has defeated the check every other protection here assumes. It composes with the path above: a hook in the working tree, hidden from `git status`, is one the user never sees appear. Nothing legitimate writes the file from inside a session (git creates it once from the template at `git init`), so the deny is free. `.gitignore` in the working tree is deliberately not protected — it is committed, so it shows up in a diff. The other route to the same concealment, `core.excludesFile`, is closed by paths already denied: `<gitdir>/config`, and the user-level `~/.gitconfig` / `~/.gitconfig.local` / `~/.config/git/config` / `~/.config/git/ignore`, which are read-only exceptions in a home that is write-denied by default.
...
"Read-only" has to hold at the target as well, because the target can sit inside a tree the agent may write: the dotfiles repo is the project, or lies under an `allow.write` grant (#524). What holds, per platform:

- **macOS:** writes are denied at the `$HOME` path and at the resolved target, after every write grant, so the target stays read-only inside the project. The directories above the target cannot be renamed or removed, so `mv dotfiles d2 && mkdir dotfiles` cannot swap in a fresh file. The cost: moving or deleting that directory fails inside the sandbox.
- **Linux with Bubblewrap:** the target is bound read-only over the writable tree, and the directories between the project, a `--repo-dir` root or an `allow.write` grant and the target are pinned as mountpoints, so they cannot be renamed.
...
On both platforms the link is resolved once, at launch, and only the file it ends at is protected. Three things fall outside that. A link whose target does not exist at launch is not covered, so the agent can create the file. An intermediate link in a chain (`~/.gitconfig -> link -> file`) that lives in a writable tree can be re-pointed. And on Linux, if `~/.config/git` itself is a link into a writable tree, only `config` in it is kept read-only; `ignore` and `attributes` there stay writable.
