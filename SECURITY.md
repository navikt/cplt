...
- **Every platform, always on: an end-of-session check.** Before the session, cplt records every `.git` below the project and each named repository, using the walk described above but with no depth limit (the 20 000-directory budget stays, symlinks are not followed). Afterwards it walks again and prints an error naming each directory whose `.git` is new, or whose kind, pointer or symlink target, `config`, `config.worktree`, `commondir` or `hooks/` listing changed. The user is told not to run git there until they have checked it. Hitting the budget is reported, since an agent can create directories to push a plant past it. This is detection, not prevention: it relies on the user reading the message. It does not look inside dependency and build directories (`node_modules`, `target`, `vendor`, …) or below `allow.write` grants, and it does not look at the project root's own `.git`.
  Two variants need no ordinary `.git` and are reported too. A **case variant** such as `.GIT`: on a case-insensitive volume (the APFS default) git opens it when it looks up `.git`. The walk finds `.git` by a filesystem lookup rather than by comparing names, so it sees exactly what git sees. A **bare-repository layout**: git's discovery accepts any directory holding `HEAD`, `objects/` and `refs/` as a gitdir and obeys the `config` beside them (`safe.bareRepository` defaults to `all`). Any new directory with all three is reported. Setting `safe.bareRepository = explicit` in your global git config closes this variant outside cplt.
...
- **macOS, opt-in: `sandbox.deny_nested_git`.** Denies `file-write-create` on `^<root>/.+/\.[gG][iI][tT]$` for every writable root. That blocks a new pointer file, symlink, hard link, `mkdir`, and a file or directory renamed onto the name, in any letter case (each verified with `sandbox-exec`). It is off by default under the staged-rollout rule because it breaks `git worktree add` into the project and fixtures that create a `.git` there. **Residual:** it does not block a bare-repository layout. A profile rule cannot say "a `config` next to a `HEAD`", and denying the names `HEAD`, `objects`, `refs` or `config` below the project would break far more than it protects. Only the end-of-session check covers that variant.
