...
- **The remote URL is not an authenticity signal.** It is attacker-controllable. A malicious repo can `git remote set-url origin <victim>` and copy the victim's approved `[propose]` block verbatim, so the content hash also matches, then look up the victim's trust entry and inherit its approved permissions. That is a confused-deputy escalation. To defeat it, every approval is **also bound to the absolute local checkout path** it was granted at (`repo.path`). Before applying a trust entry, cplt requires the current project directory to match that recorded path, canonicalized. A matching fingerprint presented from a *different* path is **not** auto-trusted and triggers a re-approval prompt. An attacker cannot satisfy this without already controlling the victim's exact on-disk location. Legacy entries with no recorded path are treated as unmatched, giving a one-time re-approval. The same binding is enforced when *writing*: `cplt trust accept` refuses outright when this origin's entry was granted at another checkout, since one entry per origin means approving here would both inherit and overwrite that entry. An entry whose recorded path cannot be resolved on disk is treated as orphaned and can be taken over — note that this covers more than a deleted checkout: an unmounted volume, a renamed parent or a permission-denied ancestor all read as orphaned, so a live checkout can lose its claim and its owner must re-approve on remount.
- **The binding is to the repository, not to one working tree** ([#516](https://github.com/navikt/cplt/issues/516)). `git worktree` gives one repository several checkouts, and one entry per origin can record only one path — so the second worktree was refused, and once approved it took the entry from the first. Trust ping-ponged between worktrees for as long as the workflow lasted. An approval therefore also records the shared git directory it was granted at (`repo.git_dir`), and an entry matches when the current project directory resolves to that same git dir. Every worktree of a repository reports the main checkout's `.git`, so one approval covers them all; a separate clone reports its own, so the spoofed origin above still fails — the attacker's checkout is a different repository whatever it names as `origin`. Anything git itself treats as the same repository is the same repository here, including a directory whose `.git` file points at one; that directory also gets the victim's committed `.cplt.toml`, since `[propose]` is read from `HEAD`, so it can only inherit an approval for the exact values the user reviewed. Nothing accumulates: it is still one entry, and the git dir is re-recorded on each approval rather than appended. Entries written before the field resolve the repository from `repo.path` instead, so an existing approval covers its worktrees without a re-approval. **What still prompts, correctly:** a worktree on a branch that changed `[propose]`, because the content hash is what pins the values and it is untouched; and a repository with no `origin`, whose worktrees are fingerprinted by path and so hold separate entries that never overwrite each other.
