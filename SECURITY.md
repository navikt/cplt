...
- **macOS:** writes are denied at the `$HOME` path and at the resolved target, after every write grant, so the target stays read-only inside the project. The directories between the outermost writable tree holding the target and the target cannot be renamed or removed, so `mv dotfiles d2 && mkdir dotfiles` cannot swap in a fresh file. The writable root itself (the project, the `--repo-dir` root, the `allow.write` grant) is not pinned: moving it away is harmless, because recreating it needs write on its parent, which no grant gives. If `~/.config/git` is itself a link, `ignore` and `attributes` at its target are write-denied even when they do not exist yet. A target whose path cannot be written into the profile (it contains `"`, `(`, `;` and the like) and sits in a writable tree stops cplt at launch rather than going unprotected.
