...
`skills/` is the eighth and the only one that does not auto-execute. It is denied because a skill steers the next session and names the scripts that session runs; the argument and the comparison with Claude Code are in the host-persistence table above. **Editing a skill from inside a session is now refused** — the write fails with `Operation not permitted`, and so does planting a new file in a skill directory. Installing one is unaffected, because installation runs outside cplt either way (`nav-pilot install` refuses to run inside a cplt session at all). `agents/` and `instructions/` keep the dir-wide write grant: they are Copilot's in-session authoring surface, they are prompt content in the sense `skills/` is not, and the deny is on `skills/` alone.
