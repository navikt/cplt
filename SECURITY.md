...
**Gemini.** First-time OAuth login needs `--allow-browser`. `~/.gemini/` holds auth, settings, sessions, and agents, and unlike Claude Code it has no write-denied subpaths on either platform. A compromised agent can write `settings.json`, whose `hooks.SessionStart[].command` entries auto-fire with the user's privileges the next time `gemini` runs *outside* the sandbox, and `trustedFolders.json`, which pre-trusts a directory so the project-hook confirmation never appears. Treat a sandboxed Gemini session as able to plant host-side code that runs on the next launch.
