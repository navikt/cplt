...
- **Every platform, always on: an end-of-session check.** Before the session, cplt records every `.git` below the project and each named repository, using the walk described above but with no depth limit (the 20 000-directory budget stays). Afterwards it walks again and prints an error naming each directory whose `.git` is new, or whose kind, pointer or symlink target, `config`, `config.worktree`, `commondir`, or hooks (each hook's name, symlink target and content) changed. The user is told not to run git there until they have checked it. This is detection, not prevention: it relies on the user reading the message. It does not look inside dependency and build directories (`node_modules`, `target`, `vendor`, …) or below `allow.write` grants, and it does not look at the project root's own `.git`.
  ...
  **Residuals of the check.** The launch record trusts the disk as it is at launch: a repository planted by a process that outlived the previous session's check is part of the next session's baseline and is not reported then. Git follows `include.path` and `includeIf` targets and a `core.hooksPath` directory; the check fingerprints none of those, so a change there, to a path the config already named at launch, is not reported. A config or hook longer than 1 MiB is compared by its first MiB, length, inode and change time, not its full content.
- **macOS, opt-in: `sandbox.deny_nested_git`.** Denies `file-write-create` on `^<root>/.+/\.[gG][iI][tT]$` for every writable root. That blocks creating the `.git` name itself as a new pointer file, symlink, hard link, `mkdir`, or a file or directory renamed onto the name, in any letter case (each verified with `sandbox-exec`). **Residuals:** it does not block moving in an *ancestor* of a `.git`. `mkdir -p /private/tmp/stage/sub/.git && mv /private/tmp/stage/sub <project>/a/sub` succeeds, because only `sub` is created below the project; any writable staging area outside it (`/private/tmp`, `/var/folders`) works. Nor does it block a symlink to a repository outside the project (`<project>/tools -> /private/tmp/evil`), which creates no `.git` below the root either. The end-of-session check reports both. It is off by default under the staged-rollout rule because it breaks `git worktree add` into the project and fixtures that create a `.git` there. It does not block a git-directory layout either. A profile rule cannot say "a `config` next to a `HEAD`", and denying the names `HEAD`, `objects`, `refs`, `commondir` or `config` below the project would break far more than it protects. Only the end-of-session check covers that variant.
