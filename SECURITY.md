...
- **On by default, with one guard still soft.** Both guards default to `enabled = true`. The gh guard enforces immediately: block mode, default-deny unknown subcommands, scope check, `block_auth_token`, `allow_api_write = false`. The git guard defaults to `mode = "warn"` — `prevent_push` and `prevent_force_push` are on underneath it, but a warn-mode verdict prints and then runs the command, so out of the box cplt **observes** pushes rather than stopping them. Set `git_guard.mode = "block"` (or `--preset strict`) to enforce. Opt out of either with `--no-gh-guard` / `--no-git-guard` or `enabled = false`; `--preset permissive` and `--preset full-trust` turn both off.
