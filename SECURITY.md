...
**`deny.env` precedence over token injection.** Repo-local `deny.env` is authoritative: if it denies `GH_TOKEN`, `GITHUB_TOKEN`, or `COPILOT_GITHUB_TOKEN`, cplt does not re-inject GH token values later in runtime even when `gh_guard.inject_token=true`. This is intentional to keep deny rules explicit and predictable; cplt emits a startup warning when this conflict is present.
