...
On both platforms the link is resolved once, at launch, and only the file it ends at is protected. Two things fall outside that. An intermediate link in a chain (`~/.gitconfig -> link -> file`) that lives in a writable tree can be re-pointed. And on Linux, a missing target cplt did not create stays creatable: without Bubblewrap, when creation failed, or when more than one directory on its path is missing. A new `config` can set `core.fsmonitor` or `core.hooksPath`, which run code in the next git command outside the sandbox; `ignore` is git's default excludes file, so an agent can hide files from `git status` through it.
