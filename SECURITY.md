...
- **Keychain isolation** (macOS) — Copilot may need Keychain access for auth; this is an accepted trade-off when no usable token is available by other means. `mach-lookup` is blanket because Node.js needs it for DNS, Security framework, and system services. The **clipboard** (`com.apple.pasteboard`) is reachable via the same blanket allow; use `--deny-clipboard` to add a targeted deny that blocks only the pasteboard service while leaving all others intact. **When Keychain is not needed:** cplt omits Keychain SBPL rules if it has a usable non-Keychain token source. By default (`sandbox.copilot_auth = "auto"`) it actively obtains one *before* the sandbox starts — an env token if set, otherwise `gh auth token` run in the unsandboxed parent — so the Keychain grant is dropped for ordinary runs rather than only when the user happens to export a token. `env_only` and `gh_only` narrow which source counts and refuse to launch rather than falling back to the Keychain; `keychain` opts out of pre-extraction entirely. Because the pre-extracted token then becomes the agent's only credential, it is injected as `GH_TOKEN` regardless of `gh_guard`, and all three token variables are cleared first so an inherited one cannot shadow it. The `gh auth token` call pins `--hostname github.com`, so a second logged-in host (e.g. GHES) cannot supply the token instead. If repo `deny.env` denies `GH_TOKEN` (the variable cplt injects into), `deny.env` wins and only ever tightens: cplt does not re-inject it and does *not* fall back to Keychain access, because repo `[deny]` is auto-applied without approval and the project dir is agent-writable — a token-var deny that re-granted Keychain would let agent-authored repo config widen the sandbox. (Denying only `GITHUB_TOKEN` / `COPILOT_GITHUB_TOKEN` clears those inherited vars but does not stop a fresh `GH_TOKEN` injection.) `copilot_auth` applies to Copilot on macOS only; elsewhere it is inert.
...
**`deny.env` precedence over token injection.** cplt injects the runtime token as `GH_TOKEN`, so a repo-local `deny.env` that denies `GH_TOKEN` stops the injected value from reaching the child even when `gh_guard.inject_token=true`; cplt emits a startup warning for this conflict. Denying only `GITHUB_TOKEN` / `COPILOT_GITHUB_TOKEN` clears those inherited variables but does not block a fresh `GH_TOKEN` injection — deny `GH_TOKEN` for that. Note also that `deny.env` does not yet suppress the `gh_guard.block_auth_token` token cache (`$SCRATCH/.gh-token`, served once via the `gh auth token` wrapper); closing that channel under the same deny is tracked in [#224](https://github.com/navikt/cplt/issues/224).
