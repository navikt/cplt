...
The remote HEAD symref, `refs/remotes/<remote>/HEAD`, names the branch `protect_default_branch_only` protects (GHSA-cm6f-3wjh-x9qx), and its Linux coverage has two gaps ([#417](https://github.com/navikt/cplt/issues/417)). macOS denies it with a regex matched at syscall time, so every remote is covered, including one added mid-session. Bubblewrap binds paths, not patterns, so it lists `refs/remotes/` at launch and binds each `HEAD` file it finds there. A remote created later in the session by `git remote add` and `git fetch` had no file to bind, so its symref stays writable for the rest of the run. On a Landlock-only host the symref is not protected at all: Landlock cannot subtract a path from a writable tree, which is a kernel limit. In both cases the default branch the git guard uses was captured at launch, so a rewrite does not change the current session's guard; a rewrite left on disk is what the next launch reads. There is no `packed-refs` gap: git never packs symbolic refs, so on the `files` backend the loose `HEAD` file exists whenever the symref does. The `reftable` backend is different, since the ref is not a file there and no path rule on either platform can see it; the git gate's refusal of `git remote set-head` and write-form `git symbolic-ref`, active whenever push prevention is, is the control in that case (see [docs/git-guard.md](docs/git-guard.md)). Where the deny holds and a remote HEAD is missing, `git fetch` (2.46+) prints `error: couldn't set 'refs/remotes/origin/HEAD'` and still exits 0 with every tracking branch updated. Closing the Linux divergences that are not kernel limits is tracked in [#386](https://github.com/navikt/cplt/issues/386).
