...
**Gemini.** First-time OAuth login needs `--allow-browser`. `~/.gemini/` holds auth, settings, sessions, and agents, and unlike Claude Code it has no write-denied subpaths on either platform. A compromised agent can write `settings.json`, whose `hooks.SessionStart[].hooks[].command` entries run with the user's privileges the next time `gemini` starts *outside* the sandbox: `SessionStart` fires on application startup, on resume, and after `/clear`. User-level hooks are not gated by folder trust, which covers workspace (`.gemini/`) settings, custom commands, MCP servers and extensions, and does not disable `~/.gemini/settings.json`. The agent can also write `trustedFolders.json`, the unsigned central file recording the per-folder trust decision, to pre-trust a directory so the dialog that would otherwise disclose the project's hooks never appears. Treat a sandboxed Gemini session as able to plant host-side code that runs on the next launch.
...
**Pi.** Not auto-detected: the `pi` binary name is generic and may collide with other tools, so select it explicitly with `--agent pi` or `sandbox.agent = "pi"`. `~/.pi/agent/` holds settings, auth, sessions, and themes. The managed binary dir `~/.pi/agent/bin/` (bundled `fd`, `rg`) has process-exec, and on **macOS only** an explicit `(deny file-write*)` follows the parent allow, so SBPL last-match-wins blocks write+exec persistence even though the parent is writable. Linux has no equivalent: Landlock rules are additive, so the parent's read+write unions with the child's read+exec and `~/.pi/agent/bin/` ends up **writable and executable**. `~/.pi/agent/extensions/` is writable on both platforms with no deny. Pi auto-discovers `~/.pi/agent/extensions/*.ts` and `*/index.ts` and loads them at startup without confirmation, since the project-trust gate covers only project-local `.pi/extensions`, and Pi's own documentation states that extensions run with the user's full system permissions and can execute arbitrary code. The writable `settings.json` in the same directory extends this: its `extensions` key loads code from arbitrary file paths and its `packages` key from npm or git. A file written to any of these executes unsandboxed the next time `pi` runs on the host.
...
- **Host-persistence guard.** Inside that writable config dir, `statusline.sh` and `plugins/` are explicitly write-denied. Both auto-execute the next time `claude` runs *outside* the sandbox, so a compromised agent could otherwise plant code that escapes via the next launch. `commands/`, `agents/`, and `skills/` stay writable, because Claude legitimately authors them and they run only when the user invokes them. `settings.json` stays writable too, and that is **a known gap, not a safe case**: its `hooks` key auto-fires on `SessionStart`, `UserPromptSubmit` and other events, running the configured command with the user's privileges the next time `claude` starts outside the sandbox. This is **macOS-only**. Like `DENIED_HOME_SUBPATHS`, Landlock cannot deny a subpath inside an allowed directory, so on Linux these stay writable, and the residual `mcpServers` risk in `~/.claude.json` is unenforceable there too. Treat write access to a relocated `CLAUDE_CONFIG_DIR` the same way.
...
- **Keychain isolation** (macOS). Copilot needs Keychain access for auth only when cplt could not obtain a GitHub token first, an accepted trade-off in that case. `mach-lookup` is blanket because Node.js needs it for DNS, the Security framework, and system services. The **clipboard** (`com.apple.pasteboard`) is reachable through that same blanket allow; `--deny-clipboard` adds a targeted deny that blocks only the pasteboard service and leaves all others intact.
- **Keychain grant dropped when a token is pre-extracted** (macOS, Copilot). Under the default `sandbox.copilot_auth = "auto"` cplt obtains a GitHub token *before* the sandbox starts — an env token if you set one, otherwise `gh auth token --hostname github.com` run in the unsandboxed parent — and then omits the Keychain rules entirely. The token is injected as `GH_TOKEN` regardless of `gh_guard`, since it is then the agent's only credential, and the other token variables are cleared first so an inherited one cannot shadow it. `env_only` and `gh_only` narrow which source counts and refuse to launch rather than falling back to the Keychain; `keychain` opts out of pre-extraction and keeps the grant. A repo `deny.env` on a token variable means "no GitHub credential", never a Keychain re-grant — repo `[deny]` is auto-applied and the project dir is agent-writable, so a token-var deny that re-granted the Keychain would let agent-authored config widen the sandbox. `copilot_auth` applies to Copilot on macOS only; elsewhere it is inert, and the Keychain grant for Gemini, Antigravity and Claude is unchanged. Two costs of the trade: a classic `ghp_` PAT does not count as a token (Copilot rejects those, so the grant is kept and a warning printed), and with the grant dropped a `/login` from inside the session cannot write its credential back to the Keychain — log in outside cplt, or start the session with `--copilot-auth keychain`.
...
**`deny.env` takes precedence over token injection.** Repo-local `deny.env` is authoritative: if it denies `GH_TOKEN`, `GITHUB_TOKEN`, or `COPILOT_GITHUB_TOKEN`, cplt does not inject a token later at exec time, even with `gh_guard.inject_token = true`, and does not fall back to Keychain access. cplt prints a startup warning when the conflict is present.
