...
SBPL accepts only `*` or `localhost` as the host in a `remote ip` rule. There is no way to name an address or a range, and Copilot connects to CDN-backed endpoints whose IPs change (`api.business.githubcopilot.com`, `api.githubcopilot.com`, `proxy.business.githubcopilot.com`), so enumerating them was never an option either. What is left is the port.

- Only port 443 is allowed. Every other outbound TCP port is blocked at the kernel level.
- Localhost outbound is blocked, which keeps the agent away from local services such as databases and dev servers.
- The SSH agent is blocked. Unix socket access is denied, so loaded SSH keys cannot be used.
...
- **Landlock network rules are TCP-only.** cplt handles `AccessNet::ConnectTcp` and nothing else, and Landlock's UDP support lands at ABI v10, above the v6 cplt clamps to. Outbound UDP to any host and port, and inbound UDP bind, are unrestricted at the kernel in **every** Linux mode including `proxy.forced`. The CONNECT proxy carries TCP only, so UDP never reaches it: DNS tunnelling, QUIC/HTTP-3 and plain UDP exfiltration have no cplt layer in front of them. macOS is unaffected, since SBPL `remote ip` covers UDP and `(deny default)` closes the rest.
- **Unix socket connects are not restricted.** Landlock gains that right only at ABI v9 (kernel 7.1), cplt clamps at v6, seccomp filters no socket syscall, and a read-only bubblewrap bind does not stop a `connect()`. Any socket whose path the agent can name is reachable: the SSH agent socket (only `SSH_AUTH_SOCK` being withheld stands in the way, and `/tmp` is readable), `/var/run/docker.sock`, the gpg-agent socket, `$XDG_RUNTIME_DIR/bus`. `--allow-socket` and `--allow-gpg-signing`'s socket rules are therefore **no-ops on Linux**: they grant filesystem access that `connect()` never consults.
- **D-Bus escapes the sandbox.** `XDG_RUNTIME_DIR` is on the env allowlist and `$XDG_RUNTIME_DIR/bus` is connectable (previous bullet), so `systemd-run --user <cmd>` starts a unit **outside** Landlock, seccomp and the bwrap namespaces. Reachable by default with and without bwrap, and cplt has no mitigation. Run inside a container or VM if this matters.
...
- **Some macOS flags do not apply.** `--allow-docker`, `--allow-jvm-attach`, and `--allow-cache-exec` emit warnings and are ignored on Linux.
