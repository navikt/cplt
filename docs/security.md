...
- **mise's `shims/` and `installs/` are protected only by bubblewrap.** The PATH-resolved bin directories are write-denied on both platforms ([global tool installs](known-impacts.md#global-tool-installs)), but the two mise ones sit inside the mise data dir, which must stay writable for the rest of mise's state. Landlock cannot subtract from an allowed tree, so they ride the same read-only overlay as the git-persistence paths: they hold only when `bwrap` is installed, and only for directories that already exist. Without bubblewrap this is unenforced on Linux, and a binary dropped in `~/.local/share/mise/shims` runs unsandboxed on the user's next command. `~/.bun/bin`, `~/.deno/bin` and `$PNPM_HOME` are not affected — those are enforced by not granting write to the parent at all, which Landlock expresses natively.
- **`map_exec` cannot be expressed, so three trees stay write+execute.** Landlock's `EXECUTE` right is checked on `execve()` alone — there is no `mmap(PROT_EXEC)` hook, and a dynamic loader gets there with `READ_FILE`. macOS's `file-map-executable` therefore has no narrower Landlock equivalent. Since [#243](https://github.com/navikt/cplt/issues/243) the execute bit follows `process_exec` alone, which took `.cargo/registry`, `.cargo/git`, `.gradle`, `.m2`, `.nuget`, `go/pkg` and the pnpm stores out of the write+execute class they had been in on Linux and nowhere else. `~/.gradle/jdks`, `~/.dotnet` and `~/.konan` keep `EXECUTE` — they really do run binaries out of themselves — and Landlock cannot subtract the parent's write grant, so those three remain write+execute on the Landlock-only path. The `EXEC_IN_WRITABLE` table in `sandbox_policy.rs` carries the reason for each; the `.dotnet` and `.konan` entries are unverified rather than decided, because no Linux host was available to run a real .NET or Kotlin/Native build against.
