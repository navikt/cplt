...
- Read-only, on the files only. Neither backend gets a directory grant or a write grant. This is the same per-file override an `allow.read "~/.npmrc"` line produces: on macOS a literal re-allow emitted after the default deny, on Linux a read-only Landlock rule on the file.
