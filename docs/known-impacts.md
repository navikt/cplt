...
| Creating git hooks                 | ❌ Blocked   | `.git/hooks/` is write-protected in the project, in every named repository (`--repo-dir`), in every `allow.write` grant, and in any repository nested under one, hooks run unsandboxed. On Linux this needs the Bubblewrap layer, and nested repositories are found by a bounded scan — three levels down, skipping dependency and build directories — so one deeper than that stays writable there. See [security.md](security.md) |
