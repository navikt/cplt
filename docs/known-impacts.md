...
> **Note:** these are the *overridable* credential denials (`DENIED_HOME_SUBPATHS`). The hard-deny list — `~/.netrc`, `~/.pypirc`, `~/.gem/credentials`, `~/.vault-token` — is not overridable at all. If you would rather not hand `~/.npmrc` to the agent, a project-level `.npmrc` is readable as part of the project directory and can take its token from an environment variable.
...
Or for a single run: `cplt --allow-read ~/.npmrc`. Deleting an `~/.npmrc` you do not use works just as well — see below.

**Why only yarn 1.** `~/.npmrc` is denied by default because it commonly holds a registry token. Every package manager reads it; only yarn 1 treats an *unreadable* one as fatal. Its rc reader tolerates a *missing* file and rethrows everything else:
...
**It is not only `~/.npmrc`.** yarn 1 runs the same reader over every entry in its rc scan, so `~/.yarnrc` fails identically when it exists:
...
Relocating yarn's config does not help. yarn 1's rc scan builds its own path list with hardcoded `~/.npmrc` and `~/.yarnrc` entries, and it runs before the XDG-aware `getConfigDir()` is consulted — so neither `XDG_CONFIG_HOME` nor `XDG_DATA_HOME` removes those paths from the list. yarn 2+ (berry) uses a different config loader and is unaffected.
