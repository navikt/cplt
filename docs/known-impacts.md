...
**Your dotfiles repo as the project:** when `~/.gitconfig`, `~/.gitconfig.local`, `~/.gitignore_global` or `~/.config/git/config` is a symlink into the repo cplt is running in (or into a `--repo-dir` root or an `allow.write` grant), the linked file stays read-only there, because it is the config git on the host trusts (#524). On macOS the same goes for `ignore` and `attributes` when `~/.config/git` itself is the link. So `git checkout`, `git pull`, `git stash pop` and `git reset --hard` fail when they would change a linked file, and git stops at that file, which can leave the working tree half-updated. Editors that save by writing a temporary file and renaming it over the original fail on it too, and so does moving or deleting the directory that holds it. The error is `Operation not permitted` on macOS, and `Read-only file system` or `Device or resource busy` on Linux with Bubblewrap. Run those operations outside cplt. Other files in the repo, and the repo directory itself, are unaffected. See [SECURITY.md](../SECURITY.md#layer-0-environment-variable-sanitization).
