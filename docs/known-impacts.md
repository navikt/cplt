...
| --- | --- | --- |
| Pi | Write-denied: `settings.json`, `trust.json`, `extensions/`, `npm/`, `git/`. On macOS, and on Linux **with bubblewrap**, `~/.pi/agent` is granted writable and those paths plus `bin/` are taken back by the profile tail (macOS) or a read-only re-bind (bubblewrap). On Linux **without bubblewrap** there is no mechanism to take a write back inside a granted tree, so the root is read-only instead, with write granted per subdirectory (`sessions/`, `prompts/`, `themes/`, `skills/`, `tools/`, `tmp/`) so that `bin/` is not inside a writable tree — **and Pi cannot start at all there**, because it takes a write lock (`mkdir trust.json.lock`) to read its trust store ([#449](https://github.com/navikt/cplt/issues/449)). Install bubblewrap, or run Pi outside cplt | ⚠️ `pi install`, `/trust`, and every in-session setting that persists to `settings.json`: `/model` (Ctrl+S), `/thinking`, `/settings`. Also, on Linux without bubblewrap, because the root is read-only, Pi cannot create a **new** top-level file or directory under `~/.pi/agent`: a first-ever `pi auth login` (creates `auth.json`) and the managed-binary bootstrap (Pi downloads `fd`/`rg` into `bin/` when they are not on `PATH`) have to run outside cplt. The root is read-only so that `bin/` is not inside a writable tree — on Linux that union made the managed binaries writable and executable. Do those outside cplt. Auth is unaffected once `auth.json` exists — Pi uses provider API keys via `--pass-env` |
