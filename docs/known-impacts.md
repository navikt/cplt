...
| --- | --- | --- |
| Pi | Write-denied: `settings.json`, `trust.json`, `extensions/`, `npm/`, `git/`. Separately, `~/.pi/agent` itself is read-only for its existing files, with write granted per subdirectory (`sessions/`, `prompts/`, `themes/`, `skills/`, `tools/`, `tmp/`) so that `bin/` is not inside a writable tree, and **create-only** for subdirectories so Pi can take its `mkdir`-based trust-store lock (`trust.json.lock/`) — `mkdir` of any new subdirectory name, and `rmdir` of any *empty* one, never file content | ⚠️ `pi install`, `/trust`, and every in-session setting that persists to `settings.json`: `/model` (Ctrl+S), `/thinking`, `/settings`. The root is read-only for its files (not create-only for them), so Pi cannot create a **new** top-level *file* under `~/.pi/agent`: a first-ever `pi auth login` (creates `auth.json`) and the managed-binary bootstrap (Pi downloads `fd`/`rg` into `bin/` when they are not on `PATH`) have to run outside cplt. New *directories* are allowed (the trust lock needs one; #449) — this is `mkdir`/`rmdir` only and cannot write file content, so `bin/` and the managed binaries stay unwritable. Auth is unaffected once `auth.json` exists — Pi uses provider API keys via `--pass-env`. **Under bubblewrap-active Linux the create-only grant also binds the root writable at the mount so `mkdir` is not `EROFS`; Landlock (create-only, no `WriteFile`) plus the read-only `bin/` overlay keep the managed binaries safe.** |
