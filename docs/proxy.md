...
> [!IMPORTANT]
> **Upstream mode delegates private-address defence to the upstream.** cplt forwards the target *hostname*, not an address, and the upstream resolves it itself. cplt's resolved-IP SSRF guard classifies the answer cplt's own resolver gives, which need not be the address the tunnel lands on: a split-horizon name that is public locally and private upstream passes the guard and still reaches the private host, and a name that does not resolve locally at all is forwarded with no address classification (that is how split-DNS names are meant to work here). This is deliberate — pinning the checked IP would break split-DNS and internal-name resolution, which is the reason upstream mode exists. In this mode the private-network boundary is the upstream proxy and its network segment, not cplt.
