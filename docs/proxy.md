...
**Interaction with allowed-domains.** Proxy-forced only changes kernel egress, meaning which port the sandbox permits. It does not replace domain filtering, because the proxy still enforces `allowed_domains` and `blocked_domains` on the traffic it carries. Use proxy-forced together with an allowlist to get "only these domains, and no way around the proxy". A default per-agent allowlist that would pair naturally with this mode is tracked in [#52](https://github.com/navikt/cplt/issues/52).
